...
/**
 * Class for storing a dependency graph between edges of networks and agents. This graph can be used
 * by dependency-based SSAs to only update the events for affected actions.
 */
public class EdgeDependencyGraph {
  /**
   * Lookup for dependents of an attribute. An entry {a,b,c} means that a edge b of network a has
   * the list of actions c as dependents.
   */
  ...
  public void RemoveEdge(Edge edge) {
    ...
    for (Action action : dependingActions) {
      for (Network owner : dependsOn.get(action).keySet()) {
        for (Iterator<Edge> edgeIt = dependsOn.get(action).get(owner).iterator();
            edgeIt.hasNext(); ) {
          if (edgeIt.next() == edge) {
            edgeIt.remove();
          }
  ...
  /**
   * Get all dependents of a certain edge owned by a specific network. This can be used to determine
   * which actions need to be reconsidered when an edge is removed changes.
   *
  ...
  /**
   * Get all candidate actions, whose guard or rate might be affected by the addition of an edge.
   *
   * @param newEdge The edge added. Only those actions are considered that depend on an edge that
   *     shares a node with the new edge.
  ...
  public ArrayList<Action> GetNewDependentCandidates(Edge newEdge, Network owner) {
    ...
    for (Action dep : networkDependents) {
      ...
      for (Edge edep : dependencies) {
        ...
        if (edep.getFrom() == newEdge.getFrom()
            || edep.getTo() == newEdge.getFrom()
            || edep.getFrom() == newEdge.getTo()
            || edep.getTo() == newEdge.getTo()) {
          result.add(dep);
          break;
        }
      }
