...
/**
 * Class for storing a dependency graph between attributes and agents. This
 * graph can be used by dependency-based SSAs to only update the events for affected actions.
 */
public class AttributeDependencyGraph {

	/**
	 * Lookup for dependents of an attribute. An entry {a,b,c} means that an
	 * attribute b of agent a has the list of actions c as dependents.
	 */
	 ...
	 * depends on the list of attributes c of agent b.
	 */
	...
	/**
	 * Get all dependents of a certain attribute owned by a specific agent. This can
	 * be used to determine which actions need to be reconsidered when an attribute
	 * changes.
	 * 
	...
	public void printAttributeDependents() {
		...
		for (Agent agent : dependents.keySet().stream().sorted((x, y) -> x.getId() - y.getId())
