...
/**
 * Program to execute all correctness and performance tests to reproduce the
 * results provided in the thesis.
 * 
 * Provides a simple CLI, so that testing can be limited to certain simulators.
 * This can be used in conjunction with Maven profiles to allow disabling or
 * enabling (possibly) performance-critical aspects.
 */
public class Evaluator {

	private enum Method {
		vanilla, frm, ldm, nrm, odm, frm_overhead
	}
...
//		predatorPreyCorrectnes(method);
	...
	private static void sirsCorrectness(Method method) {
		...
		switch (method) {
		case frm:
			new SSASirsCorrectnessExperiment("FirstReactionMethod", new FirstReactionMethod(), numHumans, density,
					initialInfected).run(reps);
			break;
		case ldm:
			new SSASirsCorrectnessExperiment("LogarithmicDirectMethod", new LogarithmicDirectMethod(), numHumans,
					density, initialInfected).run(reps);
			break;
		case nrm:
			new SSASirsCorrectnessExperiment("NextReactionMethod", new NextReactionMethod(), numHumans, density,
					initialInfected).run(reps);
			break;
		case odm:
			new SSASirsCorrectnessExperiment("OptimizedDirectMethod", new OptimizedDirectMethod(), numHumans, density,
					initialInfected).run(reps);
			break;
		case vanilla:
	...
	/**
	 * Simulate the model with each simulator and store the means of S,I and R
	 * population counts over time.
	 */
	private static void sirCorrectness(Method method) {
		...
		switch (method) {
		case frm:
			new SSASirCorrectnessExperiment("FirstReactionMethod", new FirstReactionMethod(), numHumans, density,
					initialInfected).run(reps);
			break;
		case ldm:
			new SSASirCorrectnessExperiment("LogarithmicDirectMethod", new LogarithmicDirectMethod(), numHumans,
					density, initialInfected).run(reps);
			break;
		case nrm:
			new SSASirCorrectnessExperiment("NextReactionMethod", new NextReactionMethod(), numHumans, density,
					initialInfected).run(reps);
			break;
		case odm:
			new SSASirCorrectnessExperiment("OptimizedDirectMethod", new OptimizedDirectMethod(), numHumans, density,
					initialInfected).run(reps);
			break;
		default:
			break;
	...
	private static void predatorPreyCorrectnes(Method method) {
		...
		switch (method) {
		case frm:
			new PredatorPreyCorrectnessExperiment("FirstReactionMethod", new FirstReactionMethod(), numWolf, numSheep)
					.run(reps);
			break;
		case ldm:
			new PredatorPreyCorrectnessExperiment("LogarithmicDirectMethod", new LogarithmicDirectMethod(), numWolf,
					numSheep).run(reps);
			break;
		case nrm:
			new PredatorPreyCorrectnessExperiment("NextReactionMethod", new NextReactionMethod(), numWolf, numSheep)
					.run(reps);
			break;
		case odm:
			new PredatorPreyCorrectnessExperiment("OptimizedDirectMethod", new OptimizedDirectMethod(), numWolf,
					numSheep).run(reps);
			break;
		default:
			break;
