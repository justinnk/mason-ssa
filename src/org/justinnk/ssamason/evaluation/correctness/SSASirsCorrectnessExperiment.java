...
public class SSASirsCorrectnessExperiment extends SirCorrectnessExperiment {
	...
	public SSASirsCorrectnessExperiment(String algorithmName, StochasticSimulationAlgorithm algorithm, int numHumans,
			double density, int initialInfected) {
	...
	protected void evaluate(DataFrame[] results) {
		for (int i = 0; i < results.length; i++) {
			CSVWriter.writeAndClose(results[i],
				Paths.get(outputPathPrefix + "correctness_data/sirs/" + this.algorithmName + "_" + i + ".csv"), true);
		}
		writeAlgoData(Paths.get(outputPathPrefix + "performance_data/sirs/" + this.algorithmName + "_algo.csv"));
