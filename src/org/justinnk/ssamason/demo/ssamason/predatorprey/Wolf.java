...
public class Wolf extends Species {
	...
	@NoAgentAttribute
	public static final int maxHunger = 3;
	...
	public Wolf(SimState model) {
		...
		}, () -> state.wolfHunt * this.hunger * state.undercoverNet.getEdges(this, null).size(), "Hunt"));
	}
	...
	private Sheep pickSheepInCircumference() {
		...
		Bag neighbors = new Bag(state.world.getNeighborsExactlyWithinDistance(state.world.getObjectLocation(this), 30));
		if (!neighbors.isEmpty()) {
			/* get the existing connections */
			Bag existingConnections = new Bag(state.undercoverNet.getEdges(this, null));
			List<Sheep> alreadyObservedSheep = (List<Sheep>) existingConnections.stream()
					.map(e -> ((Edge) e).getOtherNode(this)).collect(Collectors.toList());
			/* get all neighbouring sheep that are not already observed */
			List<Sheep> potentialNewPrey = (List<Sheep>) neighbors.stream()
