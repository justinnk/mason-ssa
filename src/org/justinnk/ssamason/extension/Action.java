...
/**
 * A class representing a single behaviour rule for an agent consisting of a
 * guard expression, an effect and a rate.
 */
public class Action {
	...
	/**
	 * The rate function that governs the sojourn time between executions of this
	 * action.
	 */
	...
	/**
	 * The value of the rate expression when it was evaluated last. This is used for
	 * calculating the new rate in the NRM.
	 */
	private double currentRate = 0.0;
	/**
	 * The value of the guard expression when it was evaluated last. This is used
	 * for recalculating the rate sum in the ODM.
	 */
	private boolean currentGuard = false;
	/**
	 * The time stamp resulting from the last evaluation of the rate expression.
	 * This is used for calculating the new rate in the NRM.
	 */
	private double currentTimestamp = Double.NEGATIVE_INFINITY;
	/**
	 * The currently scheduled event for this action. This is needs to be tracked
	 * for stopping the last event when rescheduling in the NRM.
	 */
	private TentativeStep nextEvent = null;

	/**
	 * Define a new behaviour rule for an agents. Should not be instantiated outside
	 * of a class inheriting from agent.
	 * 
	 * @param precondition  The guard function for determining whether this action is
	 *               applicable in the current simulation state.
	 * @param effect The effect function that is executed when the action is
	 *               applied.
	 * @param rate   The rate of this action governing the sojourn time between
	 *               applications.
	 */
	...
	/**
	 * Internal initialisation routine that sets the name properly and also adds
	 * this instance to the instance list.
	 */
	...
	/**
	 * If there is an event scheduled for this action, stop it. Resets the
	 * currentTimestamp to Double.NETGATIVE_INFINITY and currentRate to 0.0
	 */
	...
	/**
	 * Evaluate the guard expression for this action in the current simulation
	 * state. This will also update the currentGuard to the new value.
	 */
	...
	/**
	 * Calculate the rate function for this action in the current simulation state.
	 * This will also update the currentRate to the new value.
	 */
